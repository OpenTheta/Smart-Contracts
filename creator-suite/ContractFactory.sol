...
// Token starting at 1
contract ERC721IncrementalURIBurnBounty is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    address public feeAddress;
    address public burnBountyContract;

    uint public feeBasisPoints;

    constructor(address FeeAddress, uint256 FeeBasisPoints, string memory BaseURI, uint256 maxSupply, uint256 Price, string memory name, string memory symbol, address burnBounty) ERC721(name, symbol) {
        feeAddress = FeeAddress;
        baseURI = BaseURI;
        MAX_NFT_SUPPLY = maxSupply;
        price = Price;
        feeBasisPoints = FeeBasisPoints;
        burnBountyContract = burnBounty;
    }
 ...
 * @dev Gets current Price
    ...
    function reserveNFT(address _senderAddress) public {
        require(burnBountyContract == msg.sender, "Only BurnBounty contract can access");
        uint256 supply = totalSupply();
        require(supply < MAX_NFT_SUPPLY, "Sale has already ended");
        uint256 tokenId = supply+1;
        _safeMint(_senderAddress, tokenId);
        string memory id = Strings.toString(tokenId);
        _setTokenURI(tokenId, string(abi.encodePacked(id, ".json")));
    ...
    function reserveNFTById(address _senderAddress, uint256 tokenId) public {
        require(burnBountyContract == msg.sender, "Only BurnBounty contract can access");
...
// Token starting at 1
contract ERC721FixedURIBurnBounty is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    constructor(address FeeAddress, uint256 FeeBasisPoints, string memory uri, uint256 maxSupply, uint256 Price, string memory name, string memory symbol, address burnBounty) ERC721(name, symbol) {
        feeAddress = FeeAddress;
        baseURI = uri;
        MAX_NFT_SUPPLY = maxSupply;
        price = Price;
...
contract ERC721Factory is Ownable {
    ...
    address public OpenThetaAccessControlAddress;
    ...
    constructor(address FeeAddress, uint256 FeeBasisPoints, address accessControl) {
        feeBasisPoints = FeeBasisPoints;
        feeAddress = FeeAddress;
        OpenThetaAccessControlAddress = accessControl;
    }

    modifier onlyAdmin {
        require(IAccessControl(OpenThetaAccessControlAddress).hasRole("0x0", msg.sender), "only the admin can perform this action");
        _;
    }

    function buildERC721Custom(string calldata name, string calldata symbol) external {
        require(IAccessControl(OpenThetaAccessControlAddress).hasRole(CREATOR_ROLE, msg.sender), "Creator not whitelisted");
    ...
    function buildERC721IncrementalURIBurnBounty( string memory baseURI, uint256 maxSupply, uint256 price, string memory name, string memory symbol, address burnBounty) external {
        require(IAccessControl(OpenThetaAccessControlAddress).hasRole(CREATOR_ROLE, msg.sender), "Creator not whitelisted");
        string memory category = "incremental";
        _contractIds.increment();
        ERC721IncrementalURI newContract = new ERC721IncrementalURIBurnBounty(feeAddress, feeBasisPoints, baseURI, maxSupply, price, name, symbol, burnBounty);
        newContract.transferOwnership(msg.sender);
        DeployedContractsList[_contractIds.current()] = DeployedContract(
            address(newContract),
            msg.sender,
    ...
    function buildERC721FixedURIBurnBounty(string memory uri, uint256 maxSupply, uint256 price, string calldata name, string calldata symbol, address burnBounty) external {
        require(IAccessControl(OpenThetaAccessControlAddress).hasRole(CREATOR_ROLE, msg.sender), "Creator not whitelisted");
        string memory category = "fixed";
        _contractIds.increment();
        ERC721FixedURI newContract = new ERC721FixedURIBurnBounty(feeAddress, feeBasisPoints, uri, maxSupply, price, name, symbol, burnBounty);
        newContract.transferOwnership(msg.sender);
        DeployedContractsList[_contractIds.current()] = DeployedContract(
            address(newContract),
            msg.sender,
    ...
    function setAccessControlAddress(address accessControl) external onlyOwner{
        OpenThetaAccessControlAddress = accessControl;
    }
    ...
    function setFeeBasisPoints(uint256 FeeBasisPoints) public onlyAdmin {
        feeBasisPoints = FeeBasisPoints;
    }
    ...
    function getByDeployedContractId(uint256 id) public view returns (DeployedContract memory){
